...
class TestVideoQueue(unittest.TestCase):
    ...
    def test_queue_video_inital(self):
        self.videoQueue.queue_video(self.key, 'first')
        self.assertEqual(self.get_room(), {'current': 'first', 'queue': []}, 'Room filled with the current video and empty queue')

    def test_queue_video(self):
        self.add_data()
        self.assertEqual(self.get_room(), {'current': 'first', 'queue': ['second', 'third']}, 'Room filled with the current video and queue')
